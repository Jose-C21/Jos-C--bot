...
// ✅ usa la MISMA api que playvideo
...
export async function ytsearchReplyHook(sock, msg) {
  try {
    ...
    if (cachedRow && cachedRow.type === "ytsearch") {
      // existe pero quizá expiró por tiempo
    ...
    const replierKey = onlyDigits(replierJid)
    const ownerKey = onlyDigits(cache.ownerKey)

    if (ownerKey && replierKey && ownerKey !== replierKey) {
    ...
    if (t === "siguiente" || t === "next") {
      ...
      const ownerTag = cache.ownerTag || getMentionTag(sock, msg)
    ...
    try {
      const { mp4, title } = await resolveMp4(video.url)
      if (!mp4) {
        await sock.sendMessage(chatId, { text: "❌ No se pudo obtener el MP4 (API sin respuesta)." }, { quoted: msg }).catch(() => {})
        try { await sock.sendMessage(chatId, { react: { text: "❌", key: msg.key } }) } catch {}
        return true
      }

      const dl = await axios.get(mp4, { responseType: "arraybuffer", timeout: 120_000 })
      ...
      await sock.sendMessage(chatId, {
        video: buf,
        mimetype: "video/mp4",
        fileName: `${safeFileName(finalTitle)}.mp4`,
        caption:
          `*${finalTitle}*\n` +
          `\n⊱┊ ${ownerTag} 𝗔𝗾𝘂𝗶́ 𝗲𝘀𝘁𝗮́ 𝘁𝘂 𝘃𝗶́𝗱𝗲𝗼.` +
          signature(),
        mentions: cache.ownerJid ? [cache.ownerJid] : []
      }, { quoted: msg }).catch(() => {})
...
// ✅ COMANDO: ytsearch
// ─────────────────────────────────────────────
export default async function ytsearch(sock, msg, { args = [], usedPrefix = "." } = {}) {
  const chatId = msg?.key?.remoteJid
  if (!chatId) return
  ...
  const ownerKey = onlyDigits(ownerJid)
