...
// 1. Si el que escribe está AFK en ESTE grupo → quitar AFK y avisar que volvió.
// 2. Si el mensaje menciona o responde a alguien AFK en ESTE grupo → avisar.
//
// El estado es POR GRUPO — el AFK de un grupo no afecta a otros grupos.
...
// Normaliza JID — quita device suffix, funciona con @lid y @s.whatsapp.net
function normalizeJid(jid = "") {
  return String(jid).replace(/:[\d]+@/, "@").trim()
}

// Extrae menciones @etiqueta del mensaje (todos los tipos)
...
// Extrae el JID del autor del mensaje respondido (reply/quote)
...
// ── Aviso: alguien mencionó / respondió a un usuario AFK ─
...
// ── Aviso: el usuario AFK volvió ─────────────────────────

async function enviarAvisoVuelta(sock, chatId, entry, userJid) {
  const userNum  = entry.num
  const conteo   = entry.menciones || 0
  const conteoTx = conteo === 0
    ? "📢 𝗡𝗮𝗱𝗶𝗲 𝘁𝗲 𝗺𝗲𝗻𝗰𝗶𝗼𝗻ó\n𝗱𝘂𝗿𝗮𝗻𝘁𝗲 𝘁𝘂 𝗮𝘂𝘀𝗲𝗻𝗰𝗶𝗮."
    : `📢 𝗗𝘂𝗿𝗮𝗻𝘁𝗲 𝘁𝘂 𝗮𝘂𝘀𝗲𝗻𝗰𝗶𝗮\n𝘁𝗲 𝗺𝗲𝗻𝗰𝗶𝗼𝗻𝗮𝗿𝗼𝗻 *${conteo}* ${conteo === 1 ? "vez" : "veces"}.`
...
export async function afkWatcher(sock, msg) {
  try {
    ...
    // JID del que escribe (con decode para LID)
    ...
    // ─────────────────────────────────────────────────────
    // 1. ¿El que escribe tiene AFK activo EN ESTE GRUPO?
    //    → quitar y avisar que volvió
    // ─────────────────────────────────────────────────────
    let volvioDe = null

    for (const [key, entry] of Object.entries(db)) {
      // Solo entradas de este grupo
      if (entry.groupId !== chatId) continue
      if (normalizeJid(entry.jid) !== normSender) continue
    ...
    if (volvioDe) {
      ...
      console.log(`[afkWatcher] ${volvioDe.num} volvió en grupo ${chatId} → AFK eliminado`)
      // No hacemos return: el mensaje sigue procesándose normalmente
    }

    // ─────────────────────────────────────────────────────
    // 2. ¿Mencionó o respondió a alguien AFK EN ESTE GRUPO?
    // ─────────────────────────────────────────────────────
    const menciones    = getMenciones(msg)
    const quotedSender = getQuotedSender(msg)

    // Unificar menciones + autor del reply, sin duplicados
    const objetivos = [...new Set([...menciones, ...(quotedSender ? [quotedSender] : [])])]

    if (!objetivos.length) return false

    // Recargar DB (puede haber cambiado en el paso anterior)
    ...
    for (const objetivoJid of objetivos) {
      const normObjetivo = normalizeJid(objetivoJid)

      for (const [key, entry] of Object.entries(dbActual)) {
        // Solo entradas de este grupo
        if (entry.groupId !== chatId) continue
        if (normalizeJid(entry.jid) !== normObjetivo) continue

        const coolKey  = `${chatId}:${normalizeJid(entry.jid)}`
        const lastSent = global._afkCooldown[coolKey] || 0

        // Siempre contar la mención/reply en la DB
        const dbCount = readDB()
        if (dbCount[key]) {
          dbCount[key].menciones = (dbCount[key].menciones || 0) + 1
          writeDB(dbCount)
        }

        // Solo enviar aviso si pasó el cooldown (no spamear)
        ...
        console.log(`[afkWatcher] Aviso AFK → ${entry.num} | por ${jidToNumber(decodedSender)} | grupo "${groupName}"`)
        break
      }
    }
